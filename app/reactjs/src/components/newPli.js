...
import { decrementDurationTime, getMsgError, getPercentDuration } from "../helper/fonctions";
...
export default function NewPli({
    ...
    const getMediaOuvertureFiles = (name) => {
        return state.mediaOuverture[name].file.filter((f) => f !== "" && f !== null);
    };
        ...
        if (!submitting) {
            ...
            if (state.inputEmoji.value || state.sondage.value.length || getMediaFiles("images").length || getMediaFiles("video").length || getMediaFiles("music").length) {
                ...
                for (let i = 0; i < state.sondageOuverture.value.length; i++) {
                    data.append("sondageOuverture", JSON.stringify(state.sondageOuverture.value[i]));
                }
                ...
                if (countTypesMedia > 1) {
                    msgErrors({
                        msg: "Vous ne pouvez ajouter qu’un seul type de média, accompagné de texte (ou pas).",
                        submit: false,
                    });
                    return;
                }
    ...
    const isPublishPli = () => {
        return publishPli && publishPli?.user?.id && auth.isConnected && publishPli.user.id == auth.user.id;
    };
    ...
    return (
        <BlocAddPli className={`${isPublishPli() && "is-publish-pli"}`}>
                    ...
                    <div className="cadre-content-new-pli">
                        <form className="form-new-pli" onSubmit={submitPli}>
                            <div className="content-new-pli" ref={divRef}>
                                {message && <ErrorFormMessage text={message} onClick={() => setMessage(null)} />}
                                <div className="new-pli-nv1">
                                    <div className="cadre-content-pli">
                                        ...
                                        <div className="bloc-footer">
                                            <NewPliOptions state={state} setState={setState} setMessage={setMessage} />
                                ...
                                {!addOuverture && (
                                    <div className="toggle-open-ouverture" onClick={() => setAddOuverture(true)}>
                                        Ajouter une ouverture <AddIcon />
                                    </div>
                                )}

                                {addOuverture && (
                                    <div className="toggle-open-ouverture" onClick={() => deleteOuverture()}>
            ...
            {!isPublishPli() && togglePli === false && (
                ...
                >
                    {isDesktopOrLaptop ? <KeyboardArrowUpIcon /> : <AddCircleOutlineOutlinedIcon />}
                </div>
            )}
