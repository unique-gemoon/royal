...
const MediaSondageOptions = Media.hasMany(SondageOptions, { as: "options" });
...
export function findPliUserNotElapsed(req, res, next) {
    Pli.findOne({
        where: {
            createdAt: {
                [Op.gt]: db.Sequelize.literal("DATE_SUB(NOW(), INTERVAL pli.allottedTime MINUTE)"),
            },
            userId: req.user.id,
        },
    })
            ...
            if (!plis) {
                ...
                for (let i = 0; i < plis.length; i++) {
                    ...
                    const user = {
                        ...
                        isSubscribed,
                        totalSubscriptions: cpPli.user.subscribers.length || 0,
                        totalSubscribers: cpPli.user.subscriptions.length || 0,
                    };
...
export function addNotVoteSondagePli(req, res) {
    SondageNotVotes.create({
        ...
        .then((sondageNotVote) => {
            res.status(200).json({ message: "ok", sondageNotVote: { id: sondageNotVote.id } });
        })
        .catch((err) => {
            res.status(400).send({ message: err.message });
        });
