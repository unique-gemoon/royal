import { sortByKey, ucfirst } from "../middleware/functions.js";
...
const PliNotificationsPli = PliNotifications.belongsTo(Pli, { foreignKey: "pliId" });
const PliUser = Pli.belongsTo(User, { foreignKey: "userId" });
...
export function findNotificationsSubscriberNewPli(req, res, next) {
    PliNotifications.findAll({
        include: [
            {
                model: Pli,
                association: PliNotificationsPli,
                as: "pli",
                attributes: ["id"],
                include: [
                    {
        ...
        .then((notifications) => {
            for (let i = 0; i < notifications.length; i++) {
                ...
                req.notifications.push({
                    ...
                    message: ucfirst(notification.pli.user.username) + " a posté un nouveau pli",
                    createdAt: notification.createdAt,
                    seen: notification.seen,
                });
            }
...
export function findNotifications(req, res) {
    ...
    let notificationsNotSeen = allNotifications.filter((notification) => !notification.seen);

    let notifications = [];
    const total = allNotifications.length;
    if (allNotifications.length > 0) {
        notifications = [...allNotifications.sort(sortByKey("-createdAt"))].splice(start, perPage);
    }
...
export function seenNotification(req, res) {
    if (req.body.type == "newAccount") {
        User.update({ seenNotificationNewAccount: true }, { where: { id: req.user.id } })
            .then((response) => {
        ...
        SubscriberNotifications.update({ seen: true }, { where: { id: req.body.id, subscriberId: req.user.id } })
            .then((response) => {
                res.status(200).send({ message: "ok" });
            })
            .catch((err) => {
        ...
        PliNotifications.update({ seen: true }, { where: { id: req.body.id, userId: req.user.id } })
            .then((response) => {
                res.status(200).send({ message: "ok" });
            })
            .catch((err) => {
        ...
        CommentNotifications.update({ seen: true }, { where: { id: req.body.id, userId: req.user.id } })
            .then((response) => {
                res.status(200).send({ message: "ok" });
            })
            .catch((err) => {
